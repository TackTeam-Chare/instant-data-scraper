...
def clean_data(df):
    ...
    df['latitude'] = df['url'].apply(lambda x: x.split('!3d')[1].split('!')[0] if '!3d' in x else None)
    df['longitude'] = df['url'].apply(lambda x: x.split('!4d')[1].split('!')[0] if '!4d' in x else None)

    # เลือกเฉพาะคอลัมน์ที่ต้องการและตัดคอลัมน์ 'url' และ 'rating' ออก
    df_filtered = df[['name', 'description', 'location', 'latitude', 'longitude', 'category', 'service_option1', 'service_option2']].copy()
...
if __name__ == "__main__":
    ...
    output_file = 'google_cleaned.json'  # เปลี่ยนเป็นพาธไฟล์ของคุณ
    output_type = 'json'  # หรือ 'json' หากต้องการบันทึกเป็น JSON
